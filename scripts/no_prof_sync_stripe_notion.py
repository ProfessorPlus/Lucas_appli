...
def run_sync_stripe_notion_no_split(secrets_no_prof, secrets_notion, since_date=None, callback=None):
    ...
    try:
        ...
        for row in all_notion_rows:
            ...
            pid = p.get("id paiements", {}).get("number", 0) or 0
        ...
        # ===========================
        # ÉTAPE 3: Matching par famille + montant (id paiement le plus haut en priorité)
        # ===========================
        ...
        for i, sp in enumerate(stripe_payments):
            ...
            # Trouver TOUTES les lignes qui matchent famille + montant
            matching_rows = []
            for nr in notion_rows:
                ...
                if family_match and amount_match:
                    matching_rows.append(nr)
            
            # Détecter les doublons
            if len(matching_rows) > 1:
                unpaid_count = sum(1 for r in matching_rows if not r["paid"])
                if unpaid_count > 1:
                    duplicates_warning.append(
                        f"{sp['family_name']} | {sp['amount']} {sp['currency']} — "
                        f"{len(matching_rows)} ligne(s) trouvée(s) dont {unpaid_count} non payée(s)"
                    )
            
            # Trier par id paiements décroissant (le plus récent en premier)
            matching_rows.sort(key=lambda r: r["pid"], reverse=True)
            
            # Chercher la première ligne NON payée (id le plus haut)
            found_unpaid = None
            found_paid = None
            for nr in matching_rows:
                if not nr["paid"] and nr["page_id"] not in matched_ids:
                    found_unpaid = nr
                    break
                elif nr["paid"] and not found_paid:
                    found_paid = nr
            
            if found_unpaid:
                update(progress, f"✅ {sp['family_name']}")
                
                # Marquer comme payé + montant net Stripe
                update_props = {
                    "Payé ?": {"checkbox": True},
                    "Date des paiements": {"date": {"start": sp["date_payment"]}},
                }
                # Ajouter le montant net si disponible
                if sp.get("montant_net"):
                    update_props["Montant réel versé par Stripe"] = {"number": round(sp["montant_net"], 2)}
