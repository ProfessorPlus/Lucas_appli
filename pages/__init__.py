...
def _try_load_data(ctx, prefer_drive=False):
    """
    Charge les données extraites TutorBird.
    - par défaut : local puis Drive
    - si prefer_drive=True : Drive puis local
    Retourne le dict des familles ou {} si rien trouvé.
    """

    def _save_local_copy(payload):
        import json
    ...
    return _load_drive() or _load_local() if prefer_drive else _load_local() or _load_drive()
...
def page_reminders(ctx):
    ...
    if st.button("🔍 Charger les familles non payées depuis Notion", width="stretch", key="load_unpaid_notion"):
        with st.spinner("Chargement depuis Notion..."):
            unpaid_data_for_matching = _try_load_data(ctx, prefer_drive=True)
            ...
            if result["success"]:
                ...
                if unpaid_data_for_matching:
                    st.caption("Données TutorBird rechargées depuis la source la plus récente disponible (Drive prioritaire).")
            else:
                st.error(f"❌ Erreur : {result['error']}")
